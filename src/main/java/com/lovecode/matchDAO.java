...
public class matchDAO {
    ...
    public String generarXML(int idUsuario) {
        ...
        try (Connection con = conexion.getConexion();
            ...
            while (rs.next()) {
                xml.append("  <match>\n");
                xml.append("    <usuario1>").append(rs.getString(1)).append("</usuario1>\n");
                xml.append("    <usuario2>").append(rs.getString(2)).append("</usuario2>\n");
                xml.append("    <fecha>").append(rs.getString(3)).append("</fecha>\n");
                xml.append("    <tecnologias_comunes>").append(rs.getString(4)).append("</tecnologias_comunes>\n");
                xml.append("  </match>\n");
            }
        } catch (SQLException e) {
